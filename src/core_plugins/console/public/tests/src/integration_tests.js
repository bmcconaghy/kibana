...
function process_context_test(data, mapping, kb_schemes, request_line, test) {
  asyncTest(test.name, function () {
    ...
    const test_api = kb._test.loadApisFromJson(json);
    //if (kb_schemes) {
        ...
        input.autocomplete._test.getCompletions(input, input.getSession(), test.cursor, '',
          function (err, terms) {
            ...
            if (test.autoCompleteSet) {
              const expected_terms = _.map(test.autoCompleteSet, function (t) {
                if (typeof t !== 'object') {
                  t = { 'name': t };
                }
                return t;
              ...
              if (terms.length != expected_terms.length) {
                ...
                const filtered_actual_terms = _.map(terms, function (actual_term, i) {
                  const expected_term = expected_terms[i];
                  const filtered_term = {};
