...
const autocomplete_engine = require('../../src/autocomplete/engine');

let { test, module, deepEqual } = window.QUnit;
...
function testUrlContext(tokenPath, otherTokenValues, expectedContext) {
  ...
  let context = { otherTokenValues: otherTokenValues };
...
function index_test(name, tokenPath, otherTokenValues, expectedContext) {
  test(name, function () {
    let test_api = new kb._test.loadApisFromJson({
      index_test: {
        endpoints: {
          _multi_indices: {
...
index_test('Index integration 2.1', ['index1'], [],
  { indices: ['index1'], autoCompleteSet: ['_multi_indices', '_single_index'] }
);

index_test('Index integration 2.2', [
  ['index1', 'index2']
], [],
...
function type_test(name, tokenPath, otherTokenValues, expectedContext) {
  test(name, function () {
    let test_api = kb._test.loadApisFromJson({
      'type_test': {
        endpoints: {
